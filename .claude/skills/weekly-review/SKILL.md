...
The point of this run is to make sure nothing quietly slipped through the week. Read the decisions log, every stakeholder file, and the standing goals, then say plainly what closed, what's still open, what's drifting, and what needs to move before Monday instead of after.
...
- **A quiet stakeholder isn't automatically bad, so check it against their stated cadence.** A daily-relationship founder going a week without a log entry is unusual. A monthly-cadence investor going three weeks isn't. `context/people/*.md` states the relationship cadence. Use it rather than applying one rule to everyone.
...
- **If a status is ambiguous, say so instead of guessing.** If a decisions-log entry doesn't clearly say open or closed, list it under "unclear" and ask rather than picking one.
...
- **`context/people/*.md`** — for each stakeholder file, read the Interaction log entries from the last 7 days and the full Open items list regardless of date. Note the stated communication style and relationship cadence at the top of each file, which you'll need in Step 5.
...
Cross-check against Step 3's open decisions, because a lot of the time these are the same thing seen from two files. Don't report the relaunch-date decision once from `decisions-log.md` and again from Sam's file as if they're two separate items.
...
- Did anything slip, mine or someone else's, without getting flagged first? (If Step 3/4 turned up something still open past its own due date with no flag raised yet, that's a miss, so say so directly.)
...
- **Decided this week:** the two-scenario runway model (logged 2026-06-18, closed 2026-06-24). Elliot wanted burn-vs-hires visibility rather than a single projection, delivered on time. One hygiene note: Elliot's people file still shows this as an open item, the log and the file disagree, flag it for `relationship-crm` to close out in his file.
...
- **Goals check:** "flag drift before it slips," a pass so far, since Priya's hiring instinct got questioned before being actioned. But both open items are due today with no resolution yet logged. If they close by end of day, the week holds; if not, that's a real miss to name next Friday, not soften.
...
- An item's age and its due date matter more than how it reads today, so check both before deciding whether to escalate.
